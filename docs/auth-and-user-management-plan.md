...
`hono-standard-rag` にログイン機能を追加し、ローカル Wiki、Chat、Search、Settings をユーザー単位で安全に扱えるようにする。
...
`../hono-standard` から以下を hono-standard-rag の構造に合わせて移植する。
...
`hono-standard` の認証コアは移植推奨。ただし hono-standard-rag では公開登録を持たない運用が前提なので、`register` ではなく admin-only user creation と bootstrap admin を中心に設計する。
