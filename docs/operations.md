# PostgreSQL・pgvectorとHTTPサーバーの運用
...
| `/api/ready` | PostgreSQLへの接続、書き込みトランザクション、必須テーブル、vector拡張、全SQL migrationの適用を確認できる | トラフィックの受け入れ判断 |

`/api/ready`は準備不足を503で返し、内部エラーは公開しません。確認用トランザクションは必ずROLLBACKし、同時に来た準備確認は同じプローブを共有します。Docker HEALTHCHECKもこのendpointを使います。

## バックアップ

PostgreSQL client toolsをインストールした環境でcustom formatの論理バックアップを作成します。pgvectorのextension定義、document、indexも対象です。出力先は新規ファイルに限り、作成途中のファイルは検証完了まで公開しません。`pg_restore --list`に通ったファイルだけを権限600で確定します。
