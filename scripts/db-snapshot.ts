...
function databasePath(value: string): string {
	...
	if (
		...
		/^(?:libsql|https?):\/\//i.test(trimmed)
	) {
		throw new Error("A local libSQL file URL or file path is required");
	}
	const filename = trimmed.startsWith("file:")
		? decodeURIComponent(trimmed.slice("file:".length).split("?")[0] ?? "")
		: trimmed;
	if (!filename) throw new Error("A local libSQL file path is required");
	...
	try {
		...
		if (command === "backup" && !second) {
			result = await backupDatabase(
				process.env.DATABASE_URL ?? "file:./data/cloudflare-local.db",
				first,
			);
