...
このSSG overlayから生成したprofileでは、`bun run build:ssg` が `/` の静的HTMLとclient assetsを `dist-ssg/` に出力します。
...
containerは非root userで実行し、`/api/health`をDocker HEALTHCHECKに利用します。SQLite fileは`./data`へ永続化されます。
