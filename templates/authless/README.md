...
Hono APIとReact/Vite frontendを同一originで配信する、認証・protected sample・component showcaseを含まないCloudflare Workers/D1 baselineです。Bun実行時はlocal libSQL fallbackを利用できます。
...
containerは非root userで実行し、`/api/health`をDocker HEALTHCHECKに利用します。local libSQL fileは`./data`へ永続化されます。Workersへ配備する場合は`wrangler.toml`のD1 bindingを実環境へ変更してください。
