...
describe("readAppEnv", () => {
	...
	it("supports explicit HTTP runtime mode", () => {
		const env = readAppEnv({
			NODE_ENV: "production",
			APP_URL: "http://rag.example.com",
			CORS_ORIGINS: "http://rag.example.com,http://localhost:5173",
			AUTH_COOKIE_SECURE: "false",
			SECURITY_HEADERS_MODE: "http",
		});
		expect(env.appUrl).toBe("http://rag.example.com");
		expect(env.corsOrigins).toEqual([
			"http://rag.example.com",
			"http://localhost:5173",
		]);
	...
	it("supports explicit HTTPS runtime mode", () => {
		...
		expect(env.appUrl).toBe("https://rag.example.com");
		expect(env.corsOrigins).toContain("https://rag.example.com");
	...
	it("supports Azure Blob wiki storage settings", () => {
		const env = readAppEnv({
			...
			WIKI_BLOB_CONTAINER: "hono-standard-rag-wiki",
		...
		expect(env.wikiBlobContainer).toBe("hono-standard-rag-wiki");
