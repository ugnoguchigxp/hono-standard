...
declare global {
	var __ragTemplateRuntime__: Promise<unknown> | undefined;
}
...
export async function getAppRuntime(): Promise<AppRuntime> {
	if (!globalThis.__ragTemplateRuntime__) {
		globalThis.__ragTemplateRuntime__ = createRuntime().catch((error) => {
			globalThis.__ragTemplateRuntime__ = undefined;
			throw error;
		});
	}
	let runtimeValue = await globalThis.__ragTemplateRuntime__;
	if (!isRuntimeShape(runtimeValue)) {
		globalThis.__ragTemplateRuntime__ = createRuntime().catch((error) => {
			globalThis.__ragTemplateRuntime__ = undefined;
			throw error;
		});
		runtimeValue = await globalThis.__ragTemplateRuntime__;
	}
	if (!isRuntimeShape(runtimeValue)) {
	...
	if (
		...
		const refreshed = await globalThis.__ragTemplateRuntime__;
