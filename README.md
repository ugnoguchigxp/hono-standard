...
[![Turso](https://img.shields.io/badge/Turso-4FF8D2.svg?style=for-the-badge)](https://turso.tech/)
...
`bootstrap` は `.env` を用意し、dependency が未導入なら `bun install --frozen-lockfile` を実行し、libSQL database に migration を適用します。
...
| `DATABASE_URL` | no | libSQL URL。local fallback は `file:sqlite.db` | `file:sqlite.db` |
| `DATABASE_AUTH_TOKEN` | remote yes | Turso remote database auth token | unset |
...
| Command | Purpose |
| --- | --- |
| `bun run bootstrap` | clone 後の初期化。`.env` を用意し、libSQL database に migration を適用 |
...
Turso/libSQL local fallback を container で試す場合:
