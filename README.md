...
**Hono (Backend) + React & Vite (Frontend)** で構築された、Markdown 知識ベース向けの **RAG (Retrieval-Augmented Generation) app template** です。

pgvector + PostgreSQL 全文検索によるハイブリッド検索、OpenAI / Azure OpenAI の **Responses API を用いた自律エージェント検索 (Agentic Search)**、Artifact 抽出、ロールベースユーザー管理 (RBAC) を含みます。
...
| **HTTP / HTTPS 切り替え** | | | |
| `APP_URL` | | 公開 URL。HTTP/HTTPS の cookie 既定値と CORS origin に使う | `https://rag.example.com` |
| `CORS_ORIGINS` | | 追加で許可する origin。カンマ区切り | `http://rag.example.com,http://localhost:5173` |
