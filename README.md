...
Hono backend と React + Vite frontend を同一 origin で動かす、Cloudflare Workers / D1 前提の Web app template です。Drizzle のユーザー認証、httpOnly Cookie による access / refresh token、React Router ベースの画面、コンポーネント showcase を含みます。
...
| `api/db/index.ts` | Cloudflare/local libSQL-compatible Drizzle runtime |
...
| Cloudflare Workers / D1 | edge runtime and production database |
| Wrangler | local Workers/D1 development and deploy |
...
`bootstrap` は `.env` を用意し、dependency が未導入なら `bun install --frozen-lockfile` を実行し、local libSQL database に migration を適用します。Workers/D1 は `wrangler.toml` と `bun run dev:worker` / `bun run db:migrate:d1` を使います。
...
| `DATABASE_URL` | no | local libSQL URL for non-Workers CLI/dev fallback | `file:sqlite.db` |
| `DATABASE_AUTH_TOKEN` | remote yes | remote libSQL auth token when not using D1 binding | unset |
...
| Command | Purpose |
| --- | --- |
| `bun run bootstrap` | clone 後の初期化。`.env` を用意し、local libSQL database に migration を適用 |
| `bun run dev:worker` | Wrangler dev server |
| `bun run deploy` | Wrangler deploy |
| `bun run db:migrate:d1` | local D1 migration |
...
Bun fallback runtimeは1プロセスにつき1つのwritable libSQL clientを作り、書き込みを共通の`SingleWriterClient`でFIFO実行します。Workers runtimeではD1 bindingをread/write client境界へ適合させ、永続化と複数isolate間の整合性はD1へ委ねます。

refresh token rotationはfamily単位でreuseを検知して失効します。複数isolateから同じtokenが同時利用される高競合環境では、D1側のtransactional coordinationを追加するか、認証専用のDurable Objectを利用してください。

## Docker

local libSQL fallback を container で試す場合:
...
- Cloudflare D1 は auth user と refresh token 保存に使います。Bun CLI/dev fallback では local libSQL file を使います。
