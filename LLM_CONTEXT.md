...
- `overlay/ssg`。SQLite baselineへSSR buildとstatic HTML生成を追加するoverlay。
- API composition rootは `api/app/hono.ts`、Bun server entryは `api/app/server.ts`。
- Client hydration entryは `web/src/entry-client.tsx`、server render entryは `web/src/entry-server.tsx`、static generationは `scripts/build-ssg.ts`、route compositionは `web/src/router.tsx`。
...
| `web/src/entry-*.tsx`, `scripts/build-ssg.ts` | hydration、server render、static generation境界 |
...
| SSG/prerender | `scripts/build-ssg.ts`, `web/src/entry-server.tsx`, `web/src/entry-client.tsx`, `vite.config.ts` | route manifest、対象view、`package.json` |
| Showcase/style | `web/src/views/showcase-view.tsx`, `web/src/showcase-settings-context.tsx`, `web/src/showcase-table-search.ts`, `web/src/styles.css` | `web/src/entry-client.tsx`, `vite.config.ts`（style pipeline変更時のみ） |
