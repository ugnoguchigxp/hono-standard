...
- `variant/postgres` はPostgreSQLを既定とするserver runtime variant。
...
- PostgreSQL/Drizzle runtimeのpublic entryは `api/db/index.ts`、schemaは `api/db/schema.ts`、migration entryは `api/db/migrate.ts`。
