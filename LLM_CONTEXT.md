...
- `overlay/ssr` はSQLite baselineにserver renderとclient hydrationを重ねたoverlay。
- Backend composition rootは `api/app/hono.ts`、Bun server entryは `api/app/server.ts`。
- Client hydration entryは `web/src/entry-client.tsx`、SSR entryは `web/src/entry-server.tsx`、provider構成は `web/src/App.tsx`、route compositionは `web/src/router.tsx`。
...
| `web/src/entry-client.tsx`, `web/src/entry-server.tsx` | hydrationとserver renderのentry |
...
| SSR/hydration | `web/src/entry-server.tsx`, `web/src/entry-client.tsx`, `vite.config.ts` |
