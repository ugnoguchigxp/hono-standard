...
- `variant/cloudflare` はCloudflare Workers/D1をdeploy runtimeとし、Bun/libSQLをlocal fallbackに持つ。
- Backend composition rootは `api/app/hono.ts`、Workers entryは `api/worker.ts`、local Bun server entryは `api/app/server.ts`。
- Frontend entryは `web/src/main.tsx`、provider構成は `web/src/App.tsx`、route compositionは `web/src/router.tsx`。
- D1 schemaとmigrationは `api/db/schema.ts` と `drizzle/`、binding/deploy設定は `wrangler.toml`。`api/db/index.ts` と `api/db/migrate.ts` はlocal libSQL fallbackを表す。
...
| `api/worker.ts`, `wrangler.toml` | Workers entry、D1 binding、deploy configuration |
...
| Workers/D1 | `api/worker.ts`, `wrangler.toml`, `api/db/schema.ts`, `drizzle/` |
| Local DB fallback | `api/db/index.ts`, `api/db/migrate.ts` |
| Operations/readiness | `api/routes/ready.route.ts`, `docs/operations.md`, `scripts/db-snapshot.ts`, `scripts/load-check.ts` |
...
- 正本品質ゲートは`bun run verify`であり、typecheck、Biome lint/format check（テストファイルを含む）、Vitest coverage（unit/contract/integration test と threshold）、production buildを内包する。local libSQLの負荷確認は`bun run verify:load`で実行する。
