...
- DB は Cloudflare D1 / local libSQL fallback。Drizzle schema は `api/db/schema.ts`、migration は `drizzle/`。
- Backend app composition は `api/app/hono.ts`、server bootstrap は `api/app/server.ts`。
- DB runtime の public entry は `api/db/index.ts`。Cloudflare variant は D1/Workers と local libSQL fallback を分ける。
...
| `Dockerfile`, `docker-compose.yml` | Optional local fallback container runtime |
...
| Change DB runtime/variant boundary | `api/db/index.ts`, `api/db/migrate.ts`, `wrangler.toml` | `api/cli/migrate.ts`, `api/cli/auth-create-admin.ts`, `docs/template-variant-management.md` | showcase UI |
...
- Keep Cloudflare/libSQL driver-specific imports inside DB adapter/migration files. Service modules should depend on exported DB types from `api/db`.
...
| Command | Purpose |
| --- | --- |
| `bun run bootstrap` | Prepare `.env`, dependencies, and local fallback migrations after clone |
...
- Set `DATABASE_URL` when using a non-default local fallback database.
