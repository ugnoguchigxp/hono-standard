...
- `overlay/ssr`。SQLite baselineへReact SSR、server render、client hydrationを追加するoverlay。
- API composition rootは `api/app/hono.ts`、Bun server entryは `api/app/server.ts`。
- Client hydration entryは `web/src/entry-client.tsx`、SSR entryは `web/src/entry-server.tsx`、providerは `web/src/App.tsx`、route compositionは `web/src/router.tsx`。
...
| `web/src/entry-client.tsx`, `web/src/entry-server.tsx` | hydrationとserver renderの境界 |
...
| SSR/hydration | `web/src/entry-server.tsx`, `web/src/entry-client.tsx`, `vite.config.ts`, `package.json` | router、対象view、`api/app/hono.ts` |
