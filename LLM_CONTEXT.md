...
- `overlay/ssg` はSQLite baselineにserver render buildとstatic generationを重ねたoverlay。
- Backend composition rootは `api/app/hono.ts`、Bun server entryは `api/app/server.ts`。
- Client hydration entryは `web/src/entry-client.tsx`、server render entryは `web/src/entry-server.tsx`、static generation entryは `scripts/build-ssg.ts`。
...
| `web/src/entry-*.tsx`, `scripts/build-ssg.ts` | hydration、server render、static generationのentry |
...
| SSG/prerender | `scripts/build-ssg.ts`, `web/src/entry-server.tsx`, `web/src/entry-client.tsx`, `vite.config.ts` |
...
- 正本品質ゲートは `bun run verify` で、typecheck、Biome、unit test、coverage、client build、SSR build、static HTML生成を含む。
- dependency変更時とrelease前は `bun run audit`、browser確認は `bun run verify:e2e` を実行する。
- authless生成物もSSG entryとprerender contractを維持し、同じgateで検証する。
