...
- `variant/turso` はTurso/libSQLを既定とし、local file URLとremote URLを扱う。
...
- DB public entryとlibSQL runtimeは `api/db/index.ts`、schemaは `api/db/schema.ts`、migration entryは `api/db/migrate.ts`。
...
| Turso/libSQL | `api/db/index.ts`, `api/db/schema.ts`, `api/db/migrate.ts`, `drizzle/` |
