...
- DB runtime の public entry は `api/db/index.ts`。pgvector variant は PostgreSQL runtime と `api/db/migrate.ts` の SQL migration runner を使う。
...
| `api/db/schema.ts` | PostgreSQL + pgvector Drizzle schema |
...
| `Dockerfile`, `docker-compose.yml` | Optional PostgreSQL + pgvector container runtime |
...
- Keep PostgreSQL driver-specific imports inside DB adapter/migration files. Service modules should depend on exported DB types from `api/db`.
...
| Command | Purpose |
| --- | --- |
| `bun run bootstrap` | Prepare `.env`, dependencies, and PostgreSQL migrations after clone |
...
- Set `DATABASE_URL` when using a non-default PostgreSQL connection.
