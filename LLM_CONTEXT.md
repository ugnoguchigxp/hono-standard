...
- `variant/pgvector`。Bun + Hono API + React/Vite frontendを同一originで提供し、PostgreSQL + pgvectorを利用する。
...
- PostgreSQL/Drizzle runtimeのpublic entryは `api/db/index.ts`、vector schemaは `api/db/schema.ts`、SQL migration runnerは `api/db/migrate.ts`、migrationは `drizzle/`。
...
| Vector document API | `api/routes/documents.route.ts`, `api/db/schema.ts`, `drizzle/0002_documents.sql` | `api/app/hono.ts`, `docker-compose.yml` |
...
| DB/schema/migration | `api/db/index.ts`, `api/db/schema.ts`, `api/db/migrate.ts`, `drizzle/` | DBを使うdomain repository、`docker-compose.yml`, `drizzle.config.ts` |
