...
const toFormattedTime = (value: string): string => `${value.slice(0, 2)}:${value.slice(2, 4)}`;
  ...
  ({
    ...
    useEffect(() => {
      ...
      const handleKeyDown = (e: KeyboardEvent) => {
        if ((e.key >= '0' && e.key <= '9') || (e.code >= 'Numpad0' && e.code <= 'Numpad9')) {
          ...
          e.preventDefault();
          handleHyphenClick();
        } else if (canUseDecimal && (e.key === '.' || e.code === 'NumpadDecimal')) {
          e.preventDefault();
          handleDecimalClick();
    ...
    const additionalButton = useMemo(() => {
      if (canUseHyphen || canUseDecimal) {
        ...
        const clickHandler = canUseHyphen ? handleHyphenClick : handleDecimalClick;
        return (
          <button
            type="button"
